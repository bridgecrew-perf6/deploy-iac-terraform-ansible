...
resource "aws_vpc_peering_connection" "uswest2-useast2" {
  provider    = aws.region-master
  peer_vpc_id = aws_vpc.vpc_ohio.id
  vpc_id      = aws_vpc.vpc_master.id
  peer_region = var.region-worker
}

#Accept VPC peering request in us-east-2 from us-west-2
resource "aws_vpc_peering_connection_accepter" "accept_peering" {
  provider                  = aws.region-worker
  vpc_peering_connection_id = aws_vpc_peering_connection.uswest2-useast2.id
  auto_accept               = true
}
...
#Overwrite default route table fo VPC(Workerer) with our route table entries
